...
#M
  #融合前后存在覆盖问题
        ...
        for i in range(0, used_backbone_levels - 1):
            ...
            pre_inter_outs[i + 1] = inter_outs[i + 1] + self.downsample_convs[i](inter_outs[i])#b保留上次down-top路径特征图
...
#M
            for i in range(used_backbone_levels - 1, 0, -1):# 倒序   3，2，1
                ...
                laterals[i - 1] = laterals[i - 1] + F.interpolate(
                    laterals[i], size=prev_shape, mode='nearest') + pre_laterals[i - 1]#pre_lateral[i - 1] 保留上次循环的特征
...
#P
        # part 2: add bottom-up path  至下而上路径融合输出
            for i in range(0, used_backbone_levels - 1): # 0,1,2 不包括3
                inter_outs[i + 1] = inter_outs[i + 1] + self.downsample_convs[i](inter_outs[i]) + pre_inter_outs[i + 1] # 将上次特征融合加入
        #融合组成元组
 #=================================================================
...
#    print(feat_sizes)
    ...
    #print(len(inputs))
    print(LFP.forward(fpn_model, inputs))
